...
def main(path, new):
    ...
    split = int(n_train - (n_train * 0.0))  # Keep 10% for validation
...
class SquadroDataset(Dataset):
    def __init__(self, data):
        ...
        z = xsum / 24 * (0.5 - data[:,10]) * 2
        z = z.reshape(len(z),1)
        pi = np.ones((len(z),5)) * 0.2
        self.x = np.transpose(torch.from_numpy(data)).float()
